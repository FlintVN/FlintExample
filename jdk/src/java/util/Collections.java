...
public class Collections {
    ...
    static class UnmodifiableSequencedCollection<E> extends UnmodifiableCollection<E> implements SequencedCollection<E> {
    ...
    static class UnmodifiableSequencedSet<E> extends UnmodifiableSequencedCollection<E> implements SequencedSet<E> {
        UnmodifiableSequencedSet(SequencedSet<? extends E> s) {
            super(s);
        }
    ...
    private static class UnmodifiableMap<K,V> implements Map<K,V> {
        ...
        static class UnmodifiableEntrySet<K,V> extends UnmodifiableSet<Map.Entry<K,V>> {
            ...
            static <K, V> Consumer<Map.Entry<? extends K, ? extends V>> entryConsumer(Consumer<? super Entry<K, V>> action) {
                return new Consumer<Map.Entry<? extends K, ? extends V>>() {
                    @Override
                    public void accept(Map.Entry<? extends K, ? extends V> e) {
    ...
    private static class UnmodifiableSequencedMap<K,V> extends UnmodifiableMap<K,V> implements SequencedMap<K,V> {
    ...
    static class UnmodifiableNavigableMap<K,V> extends UnmodifiableSortedMap<K,V> implements NavigableMap<K,V> {
    ...
    static class CheckedList<E> extends CheckedCollection<E> implements List<E> {
        ...
        public void replaceAll(UnaryOperator<E> operator) {
            ...
            list.replaceAll(new UnaryOperator<E>() {
                @Override
                public E apply(E e) {
    ...
    private static class CheckedMap<K,V> implements Map<K,V> {
        ...
        public V computeIfAbsent(K key, Function<? super K, ? extends V> mappingFunction) {
            ...
            return m.computeIfAbsent(key, new Function<K, V>() {
                @Override
                public V apply(K k) {
        ...
        public V merge(K key, V value, BiFunction<? super V, ? super V, ? extends V> remappingFunction) {
            ...
            return m.merge(key, value, new BiFunction<V, V, V>() {
                @Override
        ...
        static class CheckedEntrySet<K,V> implements Set<Map.Entry<K,V>> {
            ...
            public Iterator<Map.Entry<K,V>> iterator() {
                ...
                return new Iterator<>() {
                    ...
                    public void forEachRemaining(Consumer<? super Entry<K, V>> action) {
                        Objects.requireNonNull(action);
                        i.forEachRemaining(new Consumer<Entry<K, V>>() {
                            @Override
                            public void accept(Entry<K, V> e) {
                                action.accept(checkedEntry(e, valueType));
                            }
                        });
                    }
                };
            ...
            public boolean contains(Object o) {
                return o instanceof Map.Entry<?, ?> e && s.contains((e instanceof CheckedEntry) ? e : checkedEntry(e, valueType));
            }
    ...
    private static class SingletonList<E> extends AbstractList<E> implements RandomAccess {
        @SuppressWarnings("serial")
        private final E element;
    ...
    private static class SequencedSetFromMap<E> extends SetFromMap<E> implements SequencedSet<E> {
        ...
        public void addFirst(E e) {
            map().putFirst(e, Boolean.TRUE);
        }

        public void addLast(E e) {
            map().putLast(e, Boolean.TRUE);
        }

        public E getFirst() {
            return nsee(map().firstEntry());
        }

        public E getLast() {
            return nsee(map().lastEntry());
        }

        public E removeFirst() {
            return nsee(map().pollFirstEntry());
        }

        public E removeLast() {
            return nsee(map().pollLastEntry());
        }
    }
    ...
    static class AsLIFOQueue<E> extends AbstractQueue<E> implements Queue<E> {
        @SuppressWarnings("serial")
